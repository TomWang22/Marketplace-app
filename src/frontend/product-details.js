document.addEventListener("DOMContentLoaded", async () => {
      ...
      .map(
        (relatedProduct) => `
            <div class="related-product">
                <img src="${relatedProduct.image_url}" alt="${
          relatedProduct.name
        }">
  ...
  // Check if reviews are already in local storage, if not, add sample reviews
  if (!localStorage.getItem("reviews")) {
    addSampleReviewsToLocalStorage();
  }

  // Display product details and update cart item count on page load
